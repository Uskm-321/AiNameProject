...
class CommunityRepository:
    ...
    async def delete_poll(self, poll_id: int):
        async with self.session.begin():
            ...
            if not poll:
                return None
            await self.session.execute(delete(CommunityPollVote).where(CommunityPollVote.poll_id == poll_id))
            await self.session.execute(delete(CommunityPollOption).where(CommunityPollOption.poll_id == poll_id))
